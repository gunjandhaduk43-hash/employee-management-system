...
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="d-flex align-items-center mb-3">
            <a href="{{ route('attendances.index', ['date' => $attendance->attendance_date ? \Carbon\Carbon::parse($attendance->attendance_date)->toDateString() : '']) }}" class="btn btn-sm btn-light border me-2">
                <i class="bi bi-arrow-left"></i>
            </a>
            <div>
                <h3 class="h4 fw-bold mb-0 text-dark">Edit Attendance: {{ $attendance->employee->name }}</h3>
                <p class="text-muted small mb-0">{{ $attendance->employee->employee_id }} &bull; {{ $attendance->employee->department->name ?? 'No Dept' }}</p>
            </div>
        </div>

        <div class="card card-custom border-0 shadow-sm">
            <div class="card-body p-4">
                <form action="{{ route('attendances.update', $attendance) }}" method="POST" id="attendanceForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control bg-light" value="{{ $attendance->employee->name }} ({{ $attendance->employee->employee_id }})" readonly disabled>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                            @foreach ($statuses as $stat)
                                <option value="{{ $stat }}" {{ old('status', $attendance->status) == $stat ? 'selected' : '' }}>
                                    {{ $stat }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    @php
                        $rawIn = $attendance->check_in ? substr((string) $attendance->check_in, 0, 5) : '';
                        $rawOut = $attendance->check_out ? substr((string) $attendance->check_out, 0, 5) : '';
                    @endphp
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="time"
                                       ...
                                       value="{{ old('check_in', $rawIn) }}">
                            </div>
                            <div class="small text-muted mt-1" id="check_in_12hr">Display format: 12-hour AM/PM</div>
                            @error('check_in')
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="time"
                                       ...
                                       value="{{ old('check_out', $rawOut) }}">
                            </div>
                            <div class="small text-muted mt-1" id="check_out_12hr">Display format: 12-hour AM/PM</div>
                            @error('check_out')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="remarks"
                                  ...
                                  placeholder="e.g. Regular working shift, doctor appointment, casual leave approved...">{{ old('remarks', $attendance->remarks) }}</textarea>
                        @error('remarks')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-2 border-top pt-3">
                        <a href="{{ route('attendances.index', ['date' => \Carbon\Carbon::parse($attendance->attendance_date)->toDateString()]) }}" class="btn btn-light border">Cancel</a>
